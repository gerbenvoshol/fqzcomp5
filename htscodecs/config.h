/* config.h - Configuration for integrated htscodecs */

/* Defined to 1 if the compiler can issue AVX2 instructions. */
#define HAVE_AVX2 1

/* Defined to 1 if the compiler can issue AVX512F instructions. */
#define HAVE_AVX512 1

/* Define to 1 if you have the <dlfcn.h> header file. */
#define HAVE_DLFCN_H 1

/* Define to 1 if you have the <fcntl.h> header file. */
#define HAVE_FCNTL_H 1

/* Define to 1 if you have the <inttypes.h> header file. */
#define HAVE_INTTYPES_H 1

/* Define to 1 if you have the <limits.h> header file. */
#define HAVE_LIMITS_H 1

/* Define to 1 if you have the <malloc.h> header file. */
#define HAVE_MALLOC_H 1

/* Defined to 1 if the compiler can issue popcnt instructions. */
#define HAVE_POPCNT 1

/* Defined to 1 if the compiler can issue SSE4.1 instructions. */
#define HAVE_SSE4_1 1

/* Defined to 1 if the compiler can issue SSSE3 instructions. */
#define HAVE_SSSE3 1

/* Define to 1 if you have the <stdint.h> header file. */
#define HAVE_STDINT_H 1

/* Define to 1 if you have the <stdio.h> header file. */
#define HAVE_STDIO_H 1

/* Define to 1 if you have the <stdlib.h> header file. */
#define HAVE_STDLIB_H 1

/* Define to 1 if you have the <strings.h> header file. */
#define HAVE_STRINGS_H 1

/* Define to 1 if you have the <string.h> header file. */
#define HAVE_STRING_H 1

/* Define to 1 if you have the <sys/stat.h> header file. */
#define HAVE_SYS_STAT_H 1

/* Define to 1 if you have the <sys/types.h> header file. */
#define HAVE_SYS_TYPES_H 1

/* Define to 1 if you have <sys/wait.h> that is POSIX.1 compatible. */
#define HAVE_SYS_WAIT_H 1

/* Define to 1 if you have the <unistd.h> header file. */
#define HAVE_UNISTD_H 1

/* Define if zlib is installed */
#define HAVE_ZLIB 1

/* Name of package */
#define PACKAGE "htscodecs"

/* Define to the address where bug reports for this package should be sent. */
#define PACKAGE_BUGREPORT ""

/* Define to the full name of this package. */
#define PACKAGE_NAME "htscodecs"

/* Define to the full name and version of this package. */
#define PACKAGE_STRING "htscodecs 1.2.2"

/* Define to the one symbol short name of this package. */
#define PACKAGE_TARNAME "htscodecs"

/* Define to the home page for this package. */
#define PACKAGE_URL ""

/* Define to the version of this package. */
#define PACKAGE_VERSION "1.2.2"

/* Define to 1 if all of the C90 standard headers exist. */
#define STDC_HEADERS 1

/* Version number of package */
#define VERSION "1.2.2"
